"""SOP-MLOPS-2026-002 評価カーネルの単体テスト。

試験書: test/UNIT_TEST.md 節 4.2
"""
...
from evaluate_feature import (  # noqa: E402
    cliffs_delta,
...
class FeatureMetricsTest(unittest.TestCase):
    ...
    def test_cliffs_delta_samples_large_arrays(self):
        rng = np.random.default_rng(2)
        fraud = rng.normal(loc=2.0, size=600)
        normal = rng.normal(loc=0.0, size=1200)
        delta = cliffs_delta(fraud, normal)
        self.assertTrue(math.isfinite(delta))
        self.assertGreater(abs(delta), 0.0)
    ...
    def test_nan_and_inf_are_dropped(self):
        normal = np.array([0.0, 1.0, np.nan, np.inf])
        fraud = np.array([5.0, 6.0, -np.inf, 7.0])
        result = evaluate_feature_comprehensive(normal, fraud)
        self.assertTrue(math.isfinite(result["ks_statistic"]))
        self.assertTrue(math.isfinite(result["cliffs_delta"]))
